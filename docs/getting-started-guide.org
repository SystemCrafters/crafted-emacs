...
After making any needed modifications to your =early-init.el= file as described
in the previous section, we now move on to =init.el=, which is where
most of the work of configuring Emacs will
...
*** Disabling Crafted Emacs Modules

If you no longer want to use a module provided by Crafted Emacs anymore, you can
 disable it as follows

1. Stop loading the module in your Emacs initialization file
2. If the module includes packages from a crafted-*-packages.el file, then uninstall the packages installed by the module

For example, if you previously enabled the =crafted-completions-packages.el=
and =crafted-completion-config.el= modules, find and remove the corresponding
=require= expressions from your Emacs initialization file:
...
Next, open the =crafted-completion-packages.el= file and note
all packages specified in
~(add-to-list 'package-selected-package ...)~ expressions.

Finally, uninstall these packages using either:

- The command =M-x package-delete=
- Or the =pakage.el= UI (to open the UI type =M-x list-packages=)

**** Deleting Specific Packages Installed by a Crafted Emacs Module

Sometimes you want most packages from a packages module but need to exclude
specific ones.  Rather than modifying the original module files, you can create
a customized copy as follows

1. Copy the module file from =crafted-emacs-home/modules/= to your =user-emacs-directory/custom-modules/= directory
2. Edit the copy to remove unwanted ~add-to-list~ expressions  
3. Uninstall the excluded packages using =M-x package-delete=

Emacs will load your custom copy of the module first, because the =custom-modules=
directory appears earlier in the ~load-path~ variable than the Crafted Emacs
=modules= directory.  Emacs will ignore the original module, because both modules
provide the same feature (see ~provide~ function for more info) and ~require~ only
loads each feature once.

For example, to exclude the =marginalia= package from
=crafted-completion-packages= follow these steps:

1. Copy =crafted-completion-packages.el= from =crafted-emacs-home/modules/= to
   =user-emacs-directory/custom-modules/=
2. Remove this line from your copy:
   #+begin_src emacs-lisp
   (add-to-list 'package-selected-packages 'marginalia)
   #+end_src
3. Delete the package:  =M-x package-delete RET marginalia RET=

This approach gives you fine-grained control without modifying Crafted Emacs
files directly.
