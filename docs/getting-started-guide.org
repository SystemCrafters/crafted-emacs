...
If you no longer want to use a module provided by Crafted Emacs anymore, you can
 disable it by completing the following two steps
...
For example, if you previously enabled the =crafted-completions-packages=
and =crafted-completion-config= modules, find and remove the corresponding
...
**** Disabling Only Specific Packages from a Crafted Emacs Module

If you want to keep most of a module but exclude specific packages, follow these
steps:

1. Stop loading this package module
2. Copy the =add-to-list= expressions from the modules code to you Emacs
  initialization file,
3. Remove the part that installs the unwanted package.
4. Uninstall the unwanted package.

For example, you previously enabled the =crafted-completion-packages= module,
but you no longer want to use the =marginalia= package installed by this module.

In this case, remove the corresponding =require= expression from you Emacs
initialization file:
...
Now, copy the =add-to-list= expressions from =crafted-completion-packages.el= to
...
Remove the expression mentioning =marginalia=:

#+begin_src emacs-lisp
(add-to-list 'package-selected-packages 'marginalia)
#+end_src

Finally, uninstall the package by typing
~M-x package-delete <RET> marginalia <RET>~.

This approach gives you fine-grained control over which packages are installed
while avoiding the need to modify the original module files.
