...
You might prefer this handy helper function to requiring two sets of packages
per crafted-emacs module. Add the following function to your ~init.el~:

#+begin_src emacs-lisp
  (defun crafted-emacs-load-modules (&optional modules)
    "Initialize crafted-emacs modules.

  If MODULES is non-nil, only initialize modules in the provided
  list.  Otherwise, initialize every module."
    (let ((mods (or modules '(defaults completion evil ide lisp org screencast
                                       speedbar startup ui updates workspaces
                                       writing))))
      (dolist (m mods)
...
Then update your ~init.el~ to use ~crafted-emacs-load-modules~ in place of any
crafted ~require~ statements.

#+begin_src emacs-lisp
  ;; Any extra packages on top of the ones added by crafted-emacs
