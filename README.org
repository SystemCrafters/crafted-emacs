...
- Use the value found in the =RATTIONAL_EMACS_HOME= environment
  variable.
...
If you prefer to put your rational-emacs customizations elsewhere, you
can specify the =RATTIONAL_EMACS_HOME= environment variable, for
example like this:

#+begin_src emacs-lisp

    (("rational" . ((user-emacs-directory . "~/path/to/rational-emacs")
                    (env . (("RATIONAL_EMACS_HOME" . "~/my-rational-emacs-config"))))))
