...
*** Loading the =custom.el= file
    ...
    Rational Emacs loads the =custom.el= file during the initialization
    process. As noted in the previous section, the =custom-set-variables= values
    are in the =SET for current session only= state. Those values are saved to
    the =custom.el= file just before the file is loaded after the initialization
    process is finished. Any values set through the Customization UI or through
    the other work flows, for example using the =org-agenda-to-front= or
    =org-remove-file= which write to the =custom-set-variables= form, are
    preserved as well.

*** Not loading the =custom.el= file

    You may choose not to load the =custom.el= file. Using
    =customize-set-variable= has the same effect as using the Customization UI,
    with the primary difference, the customization is not written later as if
    you had used the Customization UI. If you choose to follow this pattern, you
    may not need to load the =custom.el= file ever. There are some caveats to
    that statement: notably, using certain work flows with Org Agenda files or
    risky variables in =.dir-locals.el= which write to the =custom.el=
    file. Those workflows will update the =custom.el= file, but if it is not
    loaded, those changes will never be applied, even though they are saved.

    To not load the =custom.el= file, add the following code to your =config.el=

    #+begin_src emacs-lisp
      (remove-hook 'after-init-hook #'rational-load-custom-file)
    #+end_src
