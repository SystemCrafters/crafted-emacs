...
*** Loading the =custom.el= file
    ...
    When ~rational-load-custom-file~ is non-nil, the =custom.el= file is loaded
    /after/ the initialization process, including after the user =config.el= is
    loaded. By default, no custom file is loaded.

    The =custom-set-variables= values (as set in =init.el=) are in the =SET for
    current session only= state, unless altered by a saved customization loaded
    from =custom.el=. Any values set through the Customization UI or other work
    flows, for example by using the =org-agenda-to-front= or =org-remove-file=
    functions, which write to the =custom-set-variables= form, are preserved in
    the =custom.el= file if they are saved for future sessions (as by the
    Customization UI widget, or by code).

*** Not loading the =custom.el= file

    By default, no custom file is loaded, but any saved customizations are saved
    to =custom.el= so you may load them manually, or opt to load them
    automatically every time Emacs starts by customizing
    ~rational-load-custom-file~ and toggling the option on, or by setting a
    non-nil value in your =config.el= like exampled in =example-config.el=.
